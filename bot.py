...
def shorten_url(url):
    try:
        short = requests.get(
            f"https://earnlinks.in/api?api={SHORTNER_API}&url={requests.utils.quote(url)}&format=text", timeout=10
        )
...
async def like_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    full_verify_link = f"{FLASK_URL}/verify/{code}"
...
"
        f"🔗 Please verify yourself by completing this quiz:
👉 <a href='{short_url}'>Click here to Verify</a>

"
        f"🕒 Link valid for 6 hours only.
"
        f"✅ After verification, the like will be sent automatically!",
        parse_mode='HTML'
    )
