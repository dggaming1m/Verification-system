...
async def like_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        try:
            ...
            player_name = info.get("name", "Player-{}".format(uid[-4:]))
        except:
            player_name = "Player-{}".format(uid[-4:])
...
"
            "📛 An error occurred while trying to send likes.\n"
            "🧾 UID: `{}`\n".format(uid)
...
async def givevip_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("✅ VIP access granted to user `{}`".format(target_id), parse_mode='Markdown')
...
def run_bot():
    ...
    thread = threading.Thread(target=flask_app.run, kwargs={"host": "0.0.0.0", "port": int(os.environ.get("PORT", 5000))})
    thread.start()
...
REQUIRED_CHANNEL = os.getenv("REQUIRED_CHANNEL", "@yourchannel")  # Replace with actual

def shorten_url(url):
    try:
        short = requests.get(
            "https://earnlinks.in/api?api={}&url={requests.utils.quote(url)}&format=text".format(SHORTNER_API), timeout=10
        )
        return short.text if short.text.startswith("http") else url
    except:
        return url

async def check_channel_join(user_id, bot):
    try:
...
# Modified /like command with verification and cooldown + shortlink
async def like_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not update.message or not update.message.text:
        return
    ...
    # ✅ Channel join check
    if not await check_channel_join(user_id, context.bot):
        join_button = InlineKeyboardMarkup([
            [InlineKeyboardButton("📢 Join Channel", "https://t.me/{}".format(REQUIRED_CHANNEL.strip('@')))],
            [InlineKeyboardButton("✅ I've Joined", callback_data="joined")]
        ])
        await update.message.reply_text("🔒 Please join our channel to use this bot:", reply_markup=join_button)
        return
    ...
    try:
        ...
        await update.message.reply_text("❌ Invalid format. Use: /like <region> <uid>", parse_mode='Markdown')
        return
    ...
    if not is_vip and last_used and datetime.utcnow() - last_used < timedelta(hours=24):
        await update.message.reply_text(
            "❌ You have reached your daily request limit. ❤️‍🩹 Please wait for reset or contact @dg_gaming_1m ✓ to upgrade your vip package!.",
            parse_mode='Markdown'
        )
        return
    ...
    full_verify_link = "{}/verify/{code}".format(FLASK_URL)
    short_url = shorten_url(full_verify_link)
...
"
        "🔗 Please verify yourself by completing this quiz:
👉 <a href='{}'>Click here to Verify</a>

".format(short_url)
        "🕒 Link valid for 6 hours only.
"
        "✅ After verification, the like will be sent automatically!",
        parse_mode='HTML'
    )
