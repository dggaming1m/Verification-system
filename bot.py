...
def shorten_url(url):
    try:
        short = requests.get(
            "https://earnlinks.in/api?api={}&url={}&format=text".format(SHORTNER_API, requests.utils.quote(url)), timeout=10
        )
...
async def like_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not await check_channel_join(user_id, context.bot):
        join_button = InlineKeyboardMarkup([
            [InlineKeyboardButton("📢 Join Channel", url="https://t.me/{}".format(REQUIRED_CHANNEL.strip("@")))],
    ...
    full_verify_link = "{}/verify/{}".format(FLASK_URL, code)
...
👉 <a href='{0}'>Click here to Verify</a>

🕒 Link valid for 6 hours only.
✅ After verification, the like will be sent automatically!""".format(short_url),
    parse_mode='HTML'
)
